...
def list_apps() -> str:
    ...
    return f"Available applications: {', '.join(apps)}"
...
def launch_by_path(executable_path: str, args: str = "") -> str:
    ...
    if not executable_path.lower().endswith('.exe'):
        return "Only .exe files are allowed"
    
    if not any(executable_path.startswith(allowed_dir) for allowed_dir in allowed_dirs):
        return f"Path not in allowed directories. Allowed: {', '.join(allowed_dirs)}"
    ...
    try:
        ...
        return f"Launched: {executable_path} {args}"
    except Exception as e:
